class LineLoginApiController < ApplicationController
  ...
  def login
    ...
    response_type = 'code'
    client_id = ENV['2006417879'] #本番環境では環境変数などに保管する
  ...
  def get_line_user_id_token(code)
    ...
    options = {
      ...
      body: {
        ...
        client_id = ENV['2006417879'], # 本番環境では環境変数などに保管
        client_secret = ENV['48fc82237ee50506236cd20e1eb1450d'] # 本番環境では環境変数などに保管
      }
    }
    response = Typhoeus::Request.post(url, options)
